...
const CategoryCardYear = () => {
  ...
  //今年のカテゴリを選択
  const categorySelectYear = contents?.yearTransactions.filter(
    (data) => data.category === categoryType
  );

  //今年のカテゴリの合計
  const categorySumsYear = contents?.yearTransactions
  ...
  // const test = categorySelectYear.sort;

  const categoryYearDate = categorySelectYear!.map((data) => {
    return data.date;
  });

  const categoryYearDateSort = Object.values(categoryYearDate).sort();

  // console.log(categoryYearDateSort);

  // const test = categorySelectYear.map((datas) => {
  //   categoryYearDateSort.filter((days) => {
  //     if (days === datas.date) {
  //       return datas;
  //       console.log(datas);
  //     }
...
export default CategoryCardYear;
