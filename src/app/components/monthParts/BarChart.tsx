...
const BarChart = () => {
  ...
  const endDay = endOfMonth(new Date(contents?.currentMonth!));
  ...
  const allDataIcome = monthryBarChartIcome(contents?.monthlyTransactions!);
  const allDataExpense = monthryBarChartExpense(contents?.monthlyTransactions!);
  ...
  return (
    ...
    >
      {contents?.isLoading ? (
        <CircularProgress />
      ) : contents?.monthlyTransactions.length! > 0 ? (
