...
const CategoryChartYear = () => {
  ...
  //毎月のカテゴリを選択
  const categorySelectMonth = contents?.monthlyTransactions.filter(
    (data) => data.category === categoryType
  );

  //毎月のカテゴリの合計
  const categorySumsMoth = contents?.monthlyTransactions
              ...
              <CardContent sx={{ padding: { xs: 1, sm: 2 } }}>
                <Stack direction={"row"}>
                  <Typography fontWeight="bold">
                    {contents?.currentMonth.getMonth()! + 1}月の{categoryType}
                    の合計金額
                  </Typography>
                </Stack>
                <Typography
